    ...
    private fun createErrorMessage(e: TreeCommandException): Text {
        ...
        if (e.cause.showUsage) {
            builder.append(NEW_LINE).append(NEW_LINE)
                .append(USAGE).append(this.rootAlias).append(Text.of(YELLOW, " ", e.usageParts.joinToString(separator = " ")))

            if (e.subCommands.isNotEmpty()) {
                builder.append(SUB_COMMANDS).append(Text.joinWith(COMMA_SEPARATOR, e.subCommands.map { Text.of(YELLOW, it) }))
            }
        }
